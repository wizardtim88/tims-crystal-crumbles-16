...
const BookRecommendations: React.FC<BookRecommendationsProps> = ({
  ...
  const generateRecommendations = async () => {
    ...
    try {
      const {
        data,
        error
      } = await supabase.functions.invoke('recommend-books', {
        body: {
          reading,
          type,
          question
  ...
  if (!hasGenerated) {
    return <div className={`mt-4 ${className}`}>
        <Button onClick={generateRecommendations} disabled={isLoading} variant="outline" className="w-full border-wizard-gold/30 text-wizard-purple hover:bg-wizard-purple/10 hover:border-wizard-gold/50">
          {isLoading ? <>
  ...
  return <div className={`mt-4 space-y-4 ${className}`}>
      <div className="flex items-center gap-2 text-sm text-wizard-purple font-medium">
        <Sparkles className="h-4 w-4" />
        <span>Tim's Mystical Media Recommendations</span>
      </div>
      
      <div className="space-y-3">
        {recommendations.map((book, index) => <Card key={book.bookId} className="p-4 bg-card/90 border-wizard-gold/20 hover:border-wizard-gold/40 transition-all cursor-pointer" onClick={() => handleBookClick(book)}>
            <div className="space-y-2">
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <h4 className="font-medium text-card-foreground line-clamp-2 text-sm">
                    {book.title}
                    {book.subtitle && <span className="text-muted-foreground text-xs block">
                        {book.subtitle}
                      </span>}
                  </h4>
                  {book.creators && <p className="text-xs text-muted-foreground mt-1">
                      by {book.creators}
                    </p>}
                </div>
                {book.url && <ExternalLink className="h-4 w-4 text-wizard-purple flex-shrink-0 mt-1" />}
              </div>
              ...
              {book.description && <p className="text-xs text-muted-foreground line-clamp-2">
                  {book.description}
                </p>}
            </div>
          </Card>)}
