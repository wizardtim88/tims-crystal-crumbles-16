...
const ShareButton: React.FC<ShareButtonProps> = ({ reading }) => {
  const generateShareText = () => {
    if (reading.spread === 'single') {
      ...
      const cardNames = reading.cards.map(c => c.card.name + (c.isReversed ? ' (R)' : '')).join(' | ');
      // Get first sentence for summary
      const summary = reading.interpretation.split('.')[0] + '.';
      return `🎴 My Past•Present•Future reading: ${cardNames} - ${summary} Link in bio to try it`;
    }
  };
